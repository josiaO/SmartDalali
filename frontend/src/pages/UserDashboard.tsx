...
function UserOverview() {
  ...
  useEffect(() => {
    const fetchUserData = async () => {
      ...
      // Fetch favorited properties (PLACEHOLDER: Requires backend endpoint for user favorites)
  ...
  // Mock data for saved searches and messages - these would also come from backend APIs
  const savedSearches = [
  ...
  const stats = [
    {
      ...
      value: loadingFavorites ? <Skeleton className="h-6 w-12" /> : favoriteProperties.length.toString(),
      icon: Heart,
    ...
    },
    {
      ...
      value: loadingRecent ? <Skeleton className="h-6 w-12" /> : recentlyViewed.length.toString(),
      icon: Eye,
    ...
    {
      title: "Messages",
      value: messages.length.toString(), // Still mock, to be integrated later
      icon: MessageSquare,
    ...
    },
    {
      ...
      value: savedSearches.length.toString(), // Still mock
      icon: Search,
          ...
          <Card className="glass-effect">
            ...
            <CardContent>
              {loadingRecent ? (
                ...
                <div className="text-center py-12 text-muted-foreground">No recently viewed properties.</div>
              )}
            </CardContent>
          </Card>
        </TabsContent>
