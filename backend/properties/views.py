...
class SupportTicketViewSet(viewsets.ModelViewSet):
    ...
    def reply(self, request, pk=None):
        ...
        sender_type = 'admin' if (request.user.is_staff or request.user.is_superuser) else 'user'
        ...
        # Update ticket status if needed (e.g., user reply re-opens closed ticket)
        if sender_type == 'user' and ticket.status == 'closed':
            ticket.status = 'open'
            ticket.save(update_fields=['status'])
        elif sender_type == 'admin' and ticket.status == 'open':
            # Optionally set to in_progress if admin replies
    ...
    def close(self, request, pk=None):
        ...
        # Verify permission (User can close own, Admin can close any)
        # ModelViewSet's get_object handles basic visibility filtration already.
        # But we might want to ensure user is owner or admin (which get_queryset does).
        
        if ticket.status == 'closed':
             return Response({'message': 'Ticket is already closed'})
